...
const create = async (data, user) =>{
    ...
    if(user.role === "premium"){//Verifica si el rol del usuario es "premium"
        productData = {...data, owner: user.email};//Si el usuario es premium, agrega el campo 'owner' con el ID del usuario al objeto de datos del producto
    };
...
const deleteOne = async ( id, user ) =>{
    ...
    if(user.role === "premium" && productData.owner !== user.email){//Verifica si el usuario es "premium" y si no es el dueño del producto
    ...
    if (user.role === "admin" && productData.owner !== "admin") {//Enviar correo al usuario premium cuando el producto es eliminado
        await sendMail(productData.owner, "Product deleted", `The product ${productData.title} was deleted`);
    };
